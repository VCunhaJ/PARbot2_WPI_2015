	...
	#include <DepthSense.hxx>
		...
		pcl::PointCloud<pcl::PointXYZRGB>::Ptr new_cloudPtr(new pcl::PointCloud<pcl::PointXYZRGB>); // object for storing desired points clouds
		pcl::PointCloud<pcl::PointXYZRGB>::Ptr planePtr(new pcl::PointCloud<pcl::PointXYZRGB>);  //object for storing extracted clouds
		pcl::PointCloud<pcl::PointXYZRGB>::Ptr plane_extractedPtr(new pcl::PointCloud<pcl::PointXYZRGB>);
		...
		segmentation.setModelType(pcl::SACMODEL_PERPENDICULAR_PLANE); //configure object to look for a plane orthogonal to wall
		...
		segmentation.setOptimizeCoefficients(true); //enable model coefficient refinement 
